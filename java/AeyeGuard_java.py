#!/usr/bin/env python3
"""
AeyeGuard_java.py - Security Static Analysis Tool for Java Files

A Python-based security static analysis tool that leverages local LLM capabilities
to perform comprehensive security vulnerability detection in Java source code files.
"""
...
def validate_java_file(file_path: str) -> Path:
    """
    Validate that the input file exists and is a Java file.
    ...
    if path.suffix.lower() != '.java':
        raise ValueError(f"File must have .java extension: {file_path}")
...
def read_java_file(file_path: Path) -> str:
    """
    Read Java file with proper encoding handling.
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description='Security Static Analysis Tool for Java Files using Local LLM',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s example.java
  %(prog)s example.java --output json --output-file report.json
  %(prog)s example.java --model qwen/qwen3-coder-30b --verbose
  %(prog)s example.java --lm-studio-url http://localhost:1234 --temperature 0.1
        """
    )
...
def main() -> int:
    ...
    try:
        ...
        logger.info("=" * 60)
        logger.info("Security Static Analysis Tool for Java Files")
        logger.info("=" * 60)
        ...
        # Validate and read Java file
        logger.info(f"Validating input file: {args.input_file}")
        file_path = validate_java_file(args.input_file)

        logger.info(f"Reading Java file: {file_path}")
        java_content = read_java_file(file_path)

        lines_count = len(java_content.splitlines())
        logger.info(f"File size: {len(java_content)} bytes, {lines_count} lines")
        ...
        analysis_result = analyzer.analyze(java_content, str(file_path))
        ...
        analysis_result['metadata']['file_size_bytes'] = len(java_content)
