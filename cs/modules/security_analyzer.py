"""
Security Analyzer module for C# code analysis.
...
class SecurityAnalyzer:
    """Performs security analysis on C# code using LLM."""
    ...
    def analyze(self, code: str, file_path: str) -> Dict[str, Any]:
        """
        Perform comprehensive security analysis on C# code.
    ...
    def _build_system_prompt(self) -> str:
        ...
        return """You are an expert security analyst specializing in C# and .NET security.
Your task is to perform comprehensive static security analysis on C# code to identify potential vulnerabilities and security issues.
...
6. .NET-specific vulnerabilities (unsafe reflection, dynamic code execution, insecure XML parsing)
...
Provide your analysis in a clear, readable text format."""
    ...
    def _build_analysis_prompt(self, code: str, file_path: str, chunk_info: Optional[str] = None) -> str:
        ...
        prompt = f"""Analyze the following C# code for security vulnerabilities{chunk_text}:
...
Please provide your analysis now."""
    ...
    def _parse_text_for_summary(self, response: str) -> Dict[str, Any]:
        ...
        if total_findings > 0 and severity_total == 0:
            ...
            if re.search(r'\b(?:sql\s+injection|command\s+injection|rce|remote\s+code\s+execution|authentication\s+bypass)\b', response_lower):
                severity_counts['critical'] = total_findings
            elif re.search(r'\b(?:xss|csrf|path\s+traversal|xml\s+injection|ldap\s+injection)\b', response_lower):
